...
namespace gw {
    ...
    function mkTool(tag: string, icon: string = "", isInline = false) {
        ...
        __ct_extends(
            Tool,
            isInline ? ContentTools.Tools.Bold : ContentTools.Tools.Heading
        )
    ...
    const fixtureTools = [["undo", "redo", "remove"]]
    ...
    function copyMissing(trg: any, src: any) {
        for (let k of Object.keys(src)) {
            if (!trg.hasOwnProperty(k)) trg[k] = src[k]
        }
    }
    ...
    function resizePicture(
        ...
        let isOK =
            (isJpeg && img.src.length < 3000000) || img.src.length < 50000
        if (w == img.width && isOK) {
            return {
                url: img.src,
                w,
    ...
    function postImgFileAsync(fileObj: File, max: number) {
        return resizeFileAsync(fileObj, max, max).then(img =>
            postImageUrlAsync(img.url, fileObj.name).then((v: ImgResponse) => {
                v.w = img.w
                v.h = img.h
                return v
            })
    ...
    function imgUploader(dialog: any) {
        ...
        dialog.addEventListener("imageuploader.fileready", (ev: any) => {
            ...
            postImgFileAsync(file, maxImgSize).then(resp => {
                dialog.save(resp.url, [resp.w, resp.h], {
                    alt: file.name,
    ...
    function editImages() {
        ...
        $("*[data-background]").each((idx, e) =>
            handleElement(e, "data-background")
        )

        function handleElement(e_: Element, attrname: string) {
            let elt = $(e_)
            let orig =
                elt.attr("data-gw-orig-" + attrname) || elt.attr(attrname)
            if (!orig || !/\.(jpe?g|png)$/.exec(orig)) return
                ...
                if (!origLoaded) {
                    ...
                    let addOption = (mw: number, mh: number) => {
                        ...
                        p = p
                            ...
                            .then(img => {
                                if (!img) return
                                let size = Math.round(
                                    (img.url.length * 3) / 4 / 1024
                                )
                                if (size > 600) img.url = "NONE"
                    ...
                    p.then(() => {
                        ...
                        Object.keys(options).forEach(k => {
                            let op = $(
                                "<input type='radio' name='res'></input>"
                            )
                            ops.push(op)
                            op.attr("value", k)
    ...
    $(window).on("load", () => {
        if (!gitwedPageInfo.isEditable) return

        if (gitwedPageInfo.epub) maxImgSize = 1600
        ...
        if (metasection.length) {
            ...
            metasection.prepend(
                "<div class=gw-label>meta-information (only visible to editors)</div>"
            )
        ...
        let msgbox = $("<div id='ct-msgbox'></div>").text(
            "Editing " + gitwedPageInfo.lang
        )
        ...
        editor.addEventListener("saved", (ev: any) => {
            ...
            if (upd) {
                ...
                savePromise = savePromise
                    ...
                    .then((data: any) => {
                        if (
                            /\/new/.test(document.location.href) &&
                            data &&
                            data.id
                        ) {
                            final = () =>
            ...
            savePromise
                .then(() => {
                    ...
                    $("img").each((i, e) => {
                        ...
                        if (d && d.serverURL) ee.attr("src", d.serverURL)
                    })
                    final()
                })
                .catch((e: any) => {
        ...
        function pasteHTML(element: any, html: string, text: string) {
            ...
            for (let i = 0; i < wrapper.childNodes.length; ++i) {
                ...
                if (c.nodeType == 1) elts.push(c as HTMLElement)
            }
        ...
        function myPaste(element: any, clip: any) {
            if (clip.files && clip.files.length) {
                editor.busy(true)
                resizeFileAsync(clip.files[0], maxImgSize, maxImgSize).then(
                    res => {
                        let burl = window.URL.createObjectURL(
                            dataUriToBlob(res.url)
                        )
            ...
            if (!html) editor.paste(element, text)
            else pasteHTML(element, html, text)
        }
        ...
        let moreBtn = $(
            "<div class='ct-ignition__button ct-ignition__button--more'></div>"
        )
        moreBtn.click(() => {
...
<a href="/gw/logout?redirect=${encodeURIComponent(
                    location.pathname
                )}">Logout</a> <br>
Content language: ${gitwedPageInfo.lang} ${
                    gitwedPageInfo.isDefaultLang ? "(default)" : ""
                } <br>
All languages: ${gitwedPageInfo.availableLangs
                    .map(l => `<a href="${currPath}?setlang=${l}">${l}</a>`)
                    .join(" ")}
</p>
`
            )
            ...
            addButton("Page history", () => {
                ...
                getJsonAsync(
                    "/api/history?path=" +
                        encodeURIComponent(currPath.replace(/\/[^\/]+$/, ""))
                ).then((data: LogEntry[]) => {
                    let ch: JQuery[] = []
                    for (let e of data) {
                        ...
                        lnk.attr(
                            "href",
                            "/" + e.id + document.location.pathname
                        )
            ...
            if (gitwedPageInfo.epub) {
                ...
                addButton("Get EPUB", () => {
                    ...
                    window.location.href =
                        "/api/epub?folder=" + encodeURIComponent(folder)
                })
                addButton("Get EPUB for Kindle", () => {
                    modal.hide()
                    dialog.hide()
                    window.location.href =
                        "/api/epub?kindle=1&folder=" +
                        encodeURIComponent(folder)
                })
                let tocTempl = $("#epub-toc-template")
                    .html()
                    .replace(/^\s*<!--/, "")
                    .replace(/-->\s*$/, "")

                let tocTemplNoImg = $("#epub-toc-template-no-img")
                    .html()
                    .replace(/^\s*<!--/, "")
                    .replace(/-->\s*$/, "")
                ...
                if (tocTempl) {
                    addButton("Re-do TOC", () => {
                        ...
                        function repl(vars: any) {
                            let inner = vars["image"]
                                ? tocTempl
                                : tocTemplNoImg || tocTempl
                            return inner.replace(/@(\w+)@/g, (f, id) =>
                                htmlQuote(vars[id] || "")
                            )
                        }
                        getJsonAsync(
                            "/api/epubtoc?folder=" + encodeURIComponent(folder)
            ...
            addButton("Invite someone to edit", () => {
                ...
                if (evInfo) dir = "/center-" + evInfo.center
