...
function lookupUserAsync(email: string) {
    return getUserConfigAsync().then(cfg => {
...
export function throttle(req: express.Request, seconds: number) {
    ...
    if (!timestamps[ip]) timestamps[ip] = now - 3600
...
export function initRoutes(app: express.Express) {
    ...
    app.post("/api/invite", async (req, res) => {
        ...
        if (!cfgPath) return res.status(400).end()
        ...
        if (!u) {
            ...
            await gitfs.main.setJsonFileAsync(
                "private/users.json",
                userCfg,
        ...
        if (cfg.center) {
            await events.updateCenterAsync(
                cfg.center,
                c => {
                    if (!c.users) c.users = []
                    if (c.users.indexOf(email) < 0) c.users.push(email)
                },
                msg,
                req.appuser
            )
...
export function hasWritePermAsync(appuser: string, localUsers: string[]) {
    ...
    if (noSecurity() && appuser == "admin") return Promise.resolve(true)
    return lookupUserAsync(appuser).then(u => {
        if (!u) return false
        if (u.admin) return true
        if (!localUsers) return false
        if (localUsers.find(e => normalizeEmail(e) == appuser)) return true
        return false
    })
}

function getUserConfigAsync() {
    return gitfs.main.getTextFileAsync("private/users.json").then(t => {
