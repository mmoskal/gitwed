...
export function rewriteCSS(url: string) {
    return getAsync(url).then<string>(resp => {
        let dls: SMap<string> = {}
        let tokId = 0
        ...
        let css = resp.text.replace(/src:([^};]+);?/g, (f, src: string) => {
            if (/^\s*url\([^\)]+\.eot[^\)]*\)\s*$/.test(src)) return ""
            let numdl = 0
            src = src.trim() + ","
        ...
        return bluebird
            .map(Object.keys(dls), id =>
                getAsync(dls[id]).then(resp => {
                    console.log(
                        "DL " + dls[id] + " -> " + resp.buffer.length + " bytes"
                    )
...
function main() {
    rewriteCSS(process.argv[2]).then(res => {
