...
async function genTOCAsync(folder: string) {
    async function getPropsAsync(htmlName: string): Promise<TocProps> {
        ...
        if (/no-toc-img/i.test(flags)) image = null
        /*
        if (!image)
    ...
    res.forEach("a", e => {
        ...
        if (fileOK(href)) subpages.push(resolveHref(href))
    })
    let rr: TocProps[] = []
    for (let fn of subpages) {
        try {
            ...
            if (!/ENOENT/.test(e.message)) console.error(e.stack)
        }
    }
    return rr
}
...
async function genEPubAsync(opts: EPubOptions) {
    ...
    async function addFileAsync(n: string, data: any = null) {
        if (filePresent[n]) return "f" + filePresent[n]
        if (data === null)
            data = await gitfs
                .findRepo(folder)
                .getFileAsync(folder + "/" + n, "master")
        let m = tools.mimeLookup(n)
        if (m == "text/html") m = "application/xhtml+xml"
        fileNo++
        filePresent[n] = fileNo
        let add = ""
        if (addProps[n]) add = `properties="${addProps[n]}"`

        if (m == "image/jpeg" || m == "image/png") {
            let tmp = await img.resizeAsync(data, {
                maxWidth: /cover/.test(add) ? 2000 : 1000,
            })
            data = tmp.buffer
        }
        ...
        if (data !== 0) {
            zip.file(n, data, {
                compression: /image/.test(m) ? "STORE" : "DEFLATE",
            })
    ...
    async function expandAsync(htmlName: string) {
        ...
        if (htmlIdx != -1 && navTmpl.length) {
            ...
            if (tmpl) {
                let tt = h(tmpl)
                if (htmlIdx == 0) tt.find(".prev").addClass("disabled")
                else tt.find(".prev").attr("href", subpages[htmlIdx - 1])
                if (htmlIdx + 1 >= subpages.length)
                    tt.find(".next").addClass("disabled")
                else tt.find(".next").attr("href", subpages[htmlIdx + 1])
        ...
        if (!opf) {
            opf = opfHead.replace(/<dc:(\w+)><\/dc:[^<>]+>/g, (f, id) => {
                let v = h("div[id=gw-meta-" + id + "]")
                    .text()
                    .trim()
                if (!v) return ""
        ...
        forEach("sup", e => {
            ...
            if (m) {
                ...
                if (id == "NaN") {
                    ...
                    e.replaceWith(
                        `<a class="foot" href="${cfg.epubEndNotes}#note${id}"><sup>${m[1]}</sup></a>`
                    )
                }
                if (par.is("p") && /^\s*<sup>/.test(par.html())) {
                    ...
                    e.replaceWith(
                        `<a epub:type="footnote" class="footback" href="#footback${id}"><sup>${m[1]}</sup></a>`
                    )
                } else {
                    e.replaceWith(
                        `<a epub:type="noteref" class="foot" id="footback${id}" href="#foot${id}"><sup>${m[1]}</sup></a>`
                    )
                }
            } else {
                t = t.toLowerCase()
                if (t == "end") {
                    e.replaceWith(
                        `<span class="end"><img src="img/end.png" alt="The End."></span>`
                    )
                }
            }
        })
        ...
        if (cfg.epubSeparator)
            forEach("p", e => {
                ...
                let img = h(
                    `<img class="separator" src="${cfg.epubSeparator}" alt="* * *" />`
                )
                e.replaceWith(img)
            })
        ...
        forEach("img", e => {
            ...
            if (isSmall) e.addClass("small")
            e.removeAttr("width")
            e.removeAttr("height")
            let par = e.parent()
            if (!par.is("div") || e.hasClass("separator")) return

            let n = e.next("*")
            let tmp = h(`<figure></figure>`)
            if (isSmall) tmp.addClass("small")
            e.replaceWith(tmp)
            tmp.append(e)
            if (n.is("figcaption")) {
                tmp.append(n)
